...
sim::SimContext::SimContext(sim::SimContext &&other) noexcept :
...
{
	std::shared_ptr< sim::Constant > constant =
		std::make_shared< sim::Constant >(width, value);
...
sim::Pin *sim::SimContext::AllocaPin(bool output, sim::Width width)
{
...
{
	std::shared_ptr< sim::Splitter > splitter =
		std::make_shared< sim::Splitter >(widthIn, widthOut);
...
{
	std::shared_ptr< sim::Transistor > transistor =
		std::make_shared< sim::Transistor >(type);
...
{
	std::shared_ptr< sim::TransmissionGate > transmissionGate =
		std::make_shared< sim::TransmissionGate >();
...
sim::Clock *sim::SimContext::AllocaClock(unsigned highDuration, unsigned lowDuration, unsigned phaseOffset)
{
	std::shared_ptr< sim::Clock > clock =
		std::make_shared< sim::Clock >(highDuration, lowDuration, phaseOffset);
