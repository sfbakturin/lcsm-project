...
sim::SimContext::SimContext(sim::SimContext &&other) noexcept :
	m_components(std::move(other.m_components)), m_wires(std::move(other.m_wires))
{
}
...
void sim::SimContext::Swap(sim::SimContext &other) noexcept
{
	...
	std::swap(m_wires, other.m_wires);
}
...
sim::Splitter *sim::SimContext::AllocaSplitter(sim::Width widthIn, std::size_t widthOut)
{
	...
	m_components.push_back(spitter);
	return spitter.get();
}
sim::Transistor *sim::SimContext::AllocaTransistor(sim::TransistorType type)
{
	...
	m_components.push_back(transistor);
...
sim::TransmissionGate *sim::SimContext::AllocaTransmissionGate()
{
	...
	m_components.push_back(transmissionGate);
